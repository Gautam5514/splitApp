...
export const registerForPushNotificationsAsync = async () => {
  ...
  if (Platform.OS === "android") {
    ...
    // Separate, user-mutable channels per notification type - the same
    // "production" pattern Splitwise/most finance apps use so someone can
    // long-press a notification and turn down chat pings without losing
    // settlement/expense alerts. The backend picks one of these channelIds
    // per push (see backend/controllers/notificationController.js).
    await Notifications.setNotificationChannelAsync("expenses", {
      name: "New expenses",
      description: "When someone adds an expense to a group you're in",
    ...
    await Notifications.setNotificationChannelAsync("settlements", {
      ...
      description: "Payment claims, confirmations, and disputes",
      importance: Notifications.AndroidImportance.MAX,
      vibrationPattern: [0, 250, 250, 250],
      lightColor: "#16a34a",
      sound: "notification.wav",
    ...
    await Notifications.setNotificationChannelAsync("groups", {
      name: "Group activity",
      description: "Being added to a group, members joining, group updates",
    ...
    await Notifications.setNotificationChannelAsync("chat", {
      name: "Chat messages",
      description: "New messages in group and direct chats",
      importance: Notifications.AndroidImportance.HIGH,
      vibrationPattern: [0, 150],
      lightColor: "#4f46e5",
      sound: "notification.wav",
